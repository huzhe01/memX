...
class EncodedCode:
    ...
    def _validated_residuals(self) -> tuple[FloatArray, ...]:
        element_count = self._element_count()
        ...
        packet_ids = [encoded.packet.packet_id for encoded in self.packets]
        ...
        for position, encoded in enumerate(self.packets):
    ...
    def decode(self, packet_count: int) -> FloatArray:
        ...
        residuals = self._validated_residuals()
...
class ProgressiveCodec:
    ...
    def encode(self, handle: str, code: FloatArray) -> EncodedCode:
        ...
        if float(np.max(np.abs(flat.astype(np.float64)))) > _FLOAT16_MAX:
            raise ValueError("code exceeds the supported float16 range")
        working = flat.astype(np.float64)
        scale = max(
            float(np.max(np.abs(working))) / 127.0,
            float(np.finfo(np.float32).eps),
        )
        base = np.round(working / scale).clip(-127, 127).astype(np.int8).astype(np.float64) * scale
        base_half = base.astype("<f2")
        ...
        residual = (working - base_half.astype(np.float64)).astype(np.float32)
