...
.PHONY: help bootstrap data smoke train evaluate report
...
help:
	...
	@echo "           MASTER_ADDR MASTER_PORT DATA_ROOT RUN_ROOT CONFIG DATA_CONFIG RESUME"
...
train:
	MEMX_MODE=train DEVICE="$(DEVICE)" WORLD_SIZE="$(WORLD_SIZE)" LOCAL_WORLD_SIZE="$(LOCAL_WORLD_SIZE)" NNODES="$(NNODES)" NODE_RANK="$(NODE_RANK)" MASTER_ADDR="$(MASTER_ADDR)" MASTER_PORT="$(MASTER_PORT)" DATA_ROOT="$(DATA_ROOT)" RUN_ROOT="$(RUN_ROOT)" CONFIG="$(CONFIG)" RESUME="$(RESUME)" scripts/launch_train.sh

evaluate:
	MEMX_MODE=evaluate DEVICE="$(DEVICE)" WORLD_SIZE="$(WORLD_SIZE)" LOCAL_WORLD_SIZE="$(LOCAL_WORLD_SIZE)" NNODES="$(NNODES)" NODE_RANK="$(NODE_RANK)" MASTER_ADDR="$(MASTER_ADDR)" MASTER_PORT="$(MASTER_PORT)" DATA_ROOT="$(DATA_ROOT)" RUN_ROOT="$(RUN_ROOT)" CONFIG="$(CONFIG)" scripts/launch_train.sh
